...
export class AccountDAODatabase implements AccountDAO{
    ...
    async getByEmail(email: string) {
        const connection = pgPromise()("postgres://postgres:123456@localhost:5432/app")
        const account = await connection.query("select * from cccat15.account where email = $1", email).catch(e => { console.log(e) });;
        await connection.$pool.end();
        return account
    }
    ...
    async getById(id: string) {
        const connection = pgPromise()("postgres://postgres:123456@localhost:5432/app")
        const account = await connection.query("select * from cccat15.account where account_id = $1", id).catch(e => { console.log(e) });;
        await connection.$pool.end().catch(e => { console.log(e) });
        return account
    }
}